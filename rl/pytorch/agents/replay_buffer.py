...
import torch
import numpy as np
from util.pytorch_rl_util import Deque, MaxHeap
...
class PrioritisedReplayBuffer(MaxHeap, Deque):
    ...
    The deque keeps track of which experiences are the oldest and so tells us which ones to delete once the buffer
    starts getting full. The heap lets us quickly retrieve the experience with the max td_value.
    And the array lets us do quick random samples with probabilities equal to the proportional td errors.
    ...
    def __init__(self, hyperparameters, seed=0):
        MaxHeap.__init__(self, hyperparameters["buffer_size"], dimension_of_value_attribute=5, default_key_to_use=0)
    ...
    def update_heap_and_heap_index_to_overwrite(self):
        """
        Updates the heap by rearranging it given the new experience that was just incorporated into it.
        If we haven't reached max capacity then the new experience is added directly into the heap,
        otherwise a pointer on the heap has changed to reflect the new experience so there's no need to add it in
        """
    ...
    def update_heap_index_to_overwrite_next(self):
        """
        This updates the heap index to write over next.
        Once the buffer gets full we stop calling this function because the nodes the heap points to start being
        changed directly rather than the pointers on the heap changing
        """
        self.heap_index_to_overwrite_next += 1

    def swap_heap_elements(self, index1, index2):
        """
        Swaps two position of two heap elements and then updates the heap_index stored in the two nodes.
        We have to override this method from Max_Heap so that it also updates the heap_index variables
        """
    ...
    def sample(self, rank_based=True):
        """
        Randomly samples a batch from experiences giving a higher likelihood to experiences with a higher td error.
        It then calculates an importance sampling weight for each sampled experience,
        you can read about this in the paper: https://arxiv.org/pdf/1511.05952.pdf
        """
    ...
    def pick_experiences_based_on_proportional_td_error(self):
        ...
        deque_sample_indexes = np.random.choice(range(len(self.deques_td_errors)),
    ...
    def give_max_td_error(self):
        """
        Returns the maximum td error currently in the heap.
        Because it is a max heap this is the top element of the heap
        """
    ...
    def __len__(self):
        """
        Tells us how many experiences there are in the replay buffer.
        This number will never exceed self.max_size
        """
